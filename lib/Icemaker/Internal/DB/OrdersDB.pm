package Icemaker::Internal::DB::OrdersDB;

use Poet qw($conf);
use Moose;

use constant METHODS => (qw/id customer_id status creation_date last_updated_date/);

extends 'Icemaker::Internal::DB::BaseDB';

# Table structure
has 'id',                is => 'rw', isa => 'Int', trigger => sub { shift->_load_by_id };
has 'customer_id',       is => 'rw', isa => 'Int';
...
sub new {
    ...
    $self->_db($conf->get('db.orders_db'));
