...
use Icemaker::Internal::Customer;
use Test::More;
use Test::MockModule;
...
require_ok('Icemaker::Internal::Customer');
run_with('get_customer',$test_get_customer);
run_with('create_customer',$test_create_customer);
run_with('activate_customer',$test_activate_customer);
run_with('inactivate_customer',$test_inactivate_customer);
...
sub run_with {
    ...
    foreach my $test_name ( keys %$test ) {
        ...
        my $got = Icemaker::Internal::Customer->new($id)->$sub($args);
        is_deeply(
            $got,
            $expected,
...
sub run2_with {
    ...
    foreach my $test_name ( keys %$test ) {
        my $expected = $test->{$test_name}->{expected};
        my $args = $test->{$test_name}->{args};
        my $got = Icemaker::Internal::Customer->$sub($args);
        is_deeply(
            $got,
            $expected,
            "$test_name: " . Data::Dumper::Dumper{
